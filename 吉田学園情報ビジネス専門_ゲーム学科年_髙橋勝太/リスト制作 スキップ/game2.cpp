...
int g_nAttackCnt3 = 0;
float g_fAngle = 0.0f;
float g_fAngle2 = 0.0f;
float g_fAngle3 = 0.0f;
bool g_bAngle = false;
bool g_bStage = false;
bool g_bCountReset = false;
...
void CGame2::Update(void)
{
	...
	else
	{
		...
		{
			//CAnimasionBg::Create(D3DXVECTOR3(SCREEN_WIDTH / 2, 300.0f, 0.0f), ENEMY_SIZE + 600, ENEMY_SIZE + 400, 10, 18);
			CEnemy::Create(D3DXVECTOR3(SCREEN_WIDTH / 2, -300.0f, 0.0f), 8.0f, D3DXVECTOR3(SCREEN_WIDTH / 2, 300.0f, 0.0f), 10000, ENEMY_SIZE + 300, ENEMY_SIZE + 200, 1000, ENEMY_BOSS_01, 0);
...
void CGame2::Attack3(void)
{
	...
	////目的地に発射
	//if (nTimeGame2 >= 3 && nTimeGame2 <= 6 && nTimeCounter2 == 0)
	//{
	//	//目的地に向かう
	//	CBullet::Create2(D3DXVECTOR3(SCREEN_WIDTH / 2, 300.0f, 0.0f), posPlayer, D3DXCOLOR(0.0f, 1.0f, 0.0f, 1.0f), 10.0f, 500, 0, BULLET_SIZE, BULLET_SIZE, OBJTYPE_BULET_ENEMY2, 1);
	//}

	////螺旋発射
	//if (nTimeGame2 >= 3 && nTimeGame2 <= 6)
	//{
	//	g_nAttackCnt++;
	...
	//	{
	//		g_fAngle += 10;
	//		CBullet::InitAngleBullet2(D3DXVECTOR3(SCREEN_WIDTH / 2, 300.0f, 0.0f), 10.0f, D3DXCOLOR(0.0f, 1.0f, 0.0f, 1.0f), 2000, 0, BULLET_SIZE, BULLET_SIZE, g_fAngle, OBJTYPE_BULET_ENEMY, 1);
	//		CBullet::InitAngleBullet2(D3DXVECTOR3(SCREEN_WIDTH / 2, 300.0f, 0.0f), 15.0f, D3DXCOLOR(1.0f, 1.0f, 0.0f, 1.0f), 2000, 0, BULLET_SIZE, BULLET_SIZE, g_fAngle, OBJTYPE_BULET_ENEMY, 2);
