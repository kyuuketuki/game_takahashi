...
void CManager::Update(void)
{
	...
	if (nLifePlayer <= 0)
	{
		...
		{
			CLogo::Create(D3DXVECTOR3(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 0.0f), SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 27);
	...
	if (CManager::m_mode == MODE_GAME || CManager::m_mode == MODE_GAME2)
	{
		...
		if (g_bPause == false)
		{
			if (m_pInputKeyboard->GetTrigger(DIK_P) == true || CGamepad::IsButtonDown(P))
			{
				g_bPause = true;
			}
		}
