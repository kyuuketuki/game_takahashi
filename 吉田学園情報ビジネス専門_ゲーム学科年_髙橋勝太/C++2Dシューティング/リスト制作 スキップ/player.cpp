...
D3DXVECTOR3 g_pos;
int g_nCntPlayer;
int g_nBulletLevel = 0;
float g_fAnglePlayer;
int g_nCntPlayerBullet = 0;
int g_nCntPlayerBullet2 = 0;
int g_nCntPlayerBullet3 = 0;

float g_fAnglePlayerBullet = 0.0f;
bool g_bAnglePlayer = false;
...
HRESULT CPlayer::Init(D3DXVECTOR3 pos, int nLife, float fSizeX, float fSizeY)
{
	...
	CLogo::Create(D3DXVECTOR3(SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 0.0f), 10, 10, 5);
...
void CPlayer::Update(void)
{
	...
	//移動処理
	//Aを押したとき
	if (pInputKeyboard->GetPress(DIK_A) == true || pInputKeyboard->GetPress(DIK_LEFT) == true || CGamepad::IsButtonPush(LEFT))
	{
		//Wを押したとき
		if (pInputKeyboard->GetPress(DIK_W) == true || pInputKeyboard->GetPress(DIK_UP) == true || CGamepad::IsButtonPush(UP))
		{
		...
		//AとSを押したとき
		else if (pInputKeyboard->GetPress(DIK_S) == true || pInputKeyboard->GetPress(DIK_RIGHT) == true || CGamepad::IsButtonPush(DOWN))
		{
			pos.x += cosf(D3DX_PI) * ((m_nSpeed) / 1.5f);
			pos.y -= cosf(D3DX_PI) * ((m_nSpeed) / 1.5f);
		}
	...
	//Dを押したとき
	else if (pInputKeyboard->GetPress(DIK_D) == true || pInputKeyboard->GetPress(DIK_RIGHT) == true || CGamepad::IsButtonPush(RIGHT))
	{
		//DとWを押したとき
		if (pInputKeyboard->GetPress(DIK_W) == true || pInputKeyboard->GetPress(DIK_UP) == true || CGamepad::IsButtonPush(UP))
		{
		...
		//DとSを押したとき
		else if (pInputKeyboard->GetPress(DIK_S) == true || pInputKeyboard->GetPress(DIK_DOWN) == true || CGamepad::IsButtonPush(DOWN))
		{
			pos.x -= cosf(D3DX_PI) * ((m_nSpeed) / 1.5f);
			pos.y -= cosf(D3DX_PI) * ((m_nSpeed) / 1.5f);
		}
	...
	//Wを押したとき
	else if (pInputKeyboard->GetPress(DIK_W) == true || pInputKeyboard->GetPress(DIK_UP) == true || CGamepad::IsButtonPush(UP))
	{
		pos.y -= (m_nSpeed);
	}
	...
	//Dを押したときの加速量
	if (pInputKeyboard->GetPress(DIK_D) == true || pInputKeyboard->GetPress(DIK_RIGHT) == true || CGamepad::IsButtonPush(RIGHT))
	{
		m_move.x += sinf(g_fAnglePlayer) * 1;
	}
	...
	//Sを押したときの加速量
	if (pInputKeyboard->GetPress(DIK_S) == true || pInputKeyboard->GetPress(DIK_DOWN) == true || CGamepad::IsButtonPush(DOWN))
	{
		m_move.y += cosf(g_fAnglePlayer) * 1;
	}
	...
	{
		//弾の発射(Press)
		if (pInputKeyboard->GetPress(DIK_SPACE) == true || CGamepad::IsButtonPush(A) == true)
		{
			if (g_nCntPlayer == 0)
			{
				//サウンドSE
	...
	if (m_nType == 4)
	{
		...
		if (g_nBulletLevel == 1)
		{
			if (g_nCntPlayerBullet == 0)
			{
				//CAnimasion::UninitType(13);		//アニメーションクラスのタイプ13を消す
				//CAnimasion::UninitType(5);		//アニメーションクラスのタイプ5を消す
